...
# ==========================================
# BULUT VERİTABANI BAĞLANTI AYARLARI (TURBO MOTOR)
# ==========================================
try:
    ...
    st.error("Bağlantı linki bulunamadı. Lütfen Streamlit Cloud 'Secrets' bölümüne DB_URL şifrenizi ekleyin.")
    st.stop()
...
# Eğer tablolar yoksa oluşturur, VARSA ASLA BOZMAZ VE DOKUNMAZ.
@st.cache_resource
def init_db():
    try:
        ...
        st.error(f"Veritabanı kontrol hatası: {e}")
    ...
    if not df_takip.empty:
        ...
        if st.session_state.get("secili_uyari_kod"):
            ...
            if uyari_kod in df_gosterim["kod"].values:
                ...
                c_u1.success(f"🚨 Şu an **{uyari_isim}** inceleniyor. (Sadece bu müşteriye odaklandınız)")
        ...
        st.info("💡 **Hızlı Düzenleme:** Tablodan durum, tarih ve **bakiye** değişikliğini hızlıca yapıp 'Kaydet' butonuna basabilirsiniz.")
        ...
        if len(secili_satirlar) > 0 or len(df_gosterim) == 1:
            # Kutucuk seçilirse veya aramayla tek kişi kalırsa, uyarı kodunu (varsa) iptal et
            pass
        ...
        elif st.session_state.get("secili_uyari_kod"):
